"""
FARMS-DNI model: updates the computation of DNI used for FARMS.
...
def Pwater(Z, tau, De):
    ...
    temp = np.tanh(b[a2] / np.power(taup[a2], 2.0)) - np.tanh(
        0.9 * a[a2] * taup[a2]
    )
    Tddcld[a2] = Tddp[a2] * np.tanh(0.9 * a[a2] * taup[a2]) + Tddp[
        a2
...
def Pice(Z, tau, De):
    ...
    Tddp[b9] = (
        19.524 * np.power(umu0[b9], 2.0) - 16.5 * umu0[b9] + 4.4612
    ) * ade[b9]
    b10 = (
        (umu0 < 0.999)
        & (De > 10.0)
...
def farms_dni(F0, tau, solar_zenith_angle, De, phase, Tddclr, Ftotal, F1):
    """
    ...
    taudni[b1] = (
        0.345353 * tau[b1]
        - 0.00244671 * np.power(tau[b1], 2.0)
    ...
    Tddcld1 = TDDP(Z, taudni, De, phase)
